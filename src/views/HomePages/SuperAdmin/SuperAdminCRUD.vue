<template>
  <div class="head">
    ...
    <el-button @click="getUser(allusertypeTMP, keyword, nullTMP)">查询</el-button>
    <el-input v-model="KeyWord" placeholder="输入关键词查询（昵称）" />
    <el-button @click="getUser(allusertypeTMP, nullTMP, KeyWord)">查询</el-button>
    <br>
    ...
    <div v-for="i in allFeed" :key="i" class="item">
      ...
      <el-button @click="ChangeType(i.user_id, i.user_type)" class="change">切换账号状态</el-button>
    </div>
  </div>
  <el-pagination v-model:current-page="currentPage" :total="total" background layout="prev, pager, next"
    @current-change="getUser(1, nullTMP, nullTMP)" size="large" />
</template>
...
import { ChangeUserTypeApi, deleteUserApi, getUserListApi } from '@/api/user';
const { proxy } = getCurrentInstance()
const globalStore = useGlobalStore()

const nullTMP = ref()
const studentTMP = ref('STUDENT');
const adminTMP = ref('ADMIN');
const allusertypeTMP = ref('STUDENT,ADMIN')
...
onMounted(() => {
  //getFeedback()
  getUser(allusertypeTMP.value, nullTMP.value, nullTMP.value);
})
...
async function getUser(type, username, nickname) {
  try {
    const response = await getUserListApi(currentPage.value, type, username, nickname);
    allFeed.value = response.data.data.list;
...
async function DEL(userID) {
  ...
  try {
    ...
    errorMessage.value = ''
    getUser(nullTMP.value, nullTMP.value, nullTMP.value);
    alert('删除成功')
  } catch (err) {
...
async function ChangeType(id, typekind) {
  const newtypekind = typekind == 'STUDENT' ? "ADMIN" : "STUDENT"
  const Sure = confirm(`确认改变账号类型：${typekind}到${newtypekind}吗？`)
  if (!Sure) return

  const changeData = {
    user_type: newtypekind,
  }
  try {
    ...
    const response = await ChangeUserTypeApi(id, changeData)
    getUser(1, nullTMP.value, nullTMP.value);
    errorMessage.value = ''
    alert('改变成功')
  } catch (err) {
...
async function MakeSure(selectedType) {
  ...
  try {
    ...
    const response = await getUserListApi(selectedType, nullTMP.value, nullTMP.value);
    allFeed.value = response.data.data.list;
    keyword.value = '';
    KeyWord.value = '';
    alert('选择成功')
